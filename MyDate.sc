+ Date {
	...
	*fromSMPTEcopy { |smpte, daylightSavings = 1|
		...
		year = (theDay / 365.242199).floor.asInteger + 1970;
		...
		month = thisMonth.first.asInteger;
		day = ((dayN - (thisMonth[1] - thisMonth[2])) + 1).asInteger;
	...
	*rawSeconds { |rawSeconds = 0, daylightSavings = 1|
		// starts at 1/1/1970
		^Date.fromSMPTEcopy( SMPTE( rawSeconds ), daylightSavings );
		}	
